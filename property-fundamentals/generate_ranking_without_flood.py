...
for h in range(0,len(coordinates)):
    desirability_count[h] = (float(scaled_schools[h]*0.325) + float(scaled_burglary[h]*0.125) + float(scaled_universal_credit[h]*0.275) + float(scaled_housing_benefit[h]*0.275))
...
p2 = plt.barh(y_pos, weighted_schools_hbar, color = (0,0.39215686,0), left=weighted_burglary_hbar+weighted_universal_credit_hbar+weighted_housing_benefit_hbar) #color = (R,G,B)
p3 = plt.barh(y_pos, weighted_burglary_hbar, color = (1,0,0.07843137), left=weighted_universal_credit_hbar+weighted_housing_benefit_hbar) #color = (R,G,B)
